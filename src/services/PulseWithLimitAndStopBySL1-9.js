    ...
    currentPeriods = currentPeriods.filter((period) => {
      ...
      if (isSale) {
        ...
        if (
          ...
          const isSaleByTP = Number(data[i][high]) >= period.buy.price * (1 + TPPercentage / 100);
          period.sale.price = isSaleByTP
            ? period.buy.price * (1 + TPPercentage / 100)
            : period.buy.price * (1 - SLPercentage / 100);

          if (!isSaleByTP) {
            startDate = null;
            // console.log("P3 stop after SL", saleDate.toLocaleString());
          }
        }
        period.profit = (period.sale.price / period.buy.price) * 100 - 100;
        periods.push({ ...period });
      ...
      if (isSale) {
        ...
        if (
          ...
          if (!isSaleByTP) {
            hStartDate = null;
            startDate = null;
            // console.log("P7 stop after SL", saleDate.toLocaleString());
          }
        }
        period.profit = (period.sale.price / period.buy.price) * 100 - 100;
        periods.push({ ...period });
...
export const P8 = (data, TPPercentage, SLPercentage, maxDeals) => {
  const EMAIndex = data[0].indexOf("EMA");
  ...
  for (let i = 2; i < data.length; i++) {
    ...
    if (startDate !== null && currentPeriods.length <= maxDeals && prevPrice < prevPSAR && curPrice > curPSAR) {
      const buyDate = new Date(Number(data[i][0]) * 1000);
      period.buy = {
        time: buyDate,
        price: curPrice,
