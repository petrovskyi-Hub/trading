...
export default function AutoSelect2({
  ...
  const autoTPAndSL = () => {
    ...
    setShowResults(true);
    setBestCleanProfit(bestCleanPercentage);
    setProfitDealsForBestCleanProfit(profitDealsForBestClean);
    setBestTPForCleanProfit(bestTPForBestClean);
    setBestSLForCleanProfit(bestSLForBestClean);

    setBestProfitDeals(bestProfitDealsPercentage);
    setCleanProfitForProfitDeals(cleanForBestProfitDeals);
    setBestTPForProfitDeals(bestTPForBestDealsP);
    setBestSLForProfitDeals(bestSLForBestDealsP);
        ...
        <div className={s.result}>
          {resultProfitDeals !== 0 && (
            <p style={{ marginBottom: "15px" }}>
              Результат:{" "}
              {`Чистая прибыль ${resultCleanProfit.toFixed(
                1
              )}%, Cоотношение сделок в % прибыльных/убыточных ${resultProfitDeals.toFixed(1)} 
          (TP=${TPPercentage}, SL=${SLPercentage})`}
            </p>
          )}
          <p>
            {`Лучшая чистая прибыль ${BestCleanProfit.toFixed(
              1
            )}%, Cоотношение сделок в % прибыльных/убыточных ${ProfitDealsForBestCleanProfit.toFixed(1)} 
          (TP=${BestTPForBestCleanProfit}, SL=${BestSLForBestCleanProfit})`}
          </p>
          {BestProfitDeals !== 0 && (
            <p>
              {`Лучшее соотношение сделок в % прибыльных/убыточных ${BestProfitDeals.toFixed(
                1
              )}, Чистая прибыль ${CleanProfitForBestProfitDeals.toFixed(1)}% 
